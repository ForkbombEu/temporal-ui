...
import ExpandButton from './expandable/button.svelte';
...
const createEditorView = (isDark, expanded) => {
...
const createEditorState = (value, isDark, expanded) => {
    const extensions = [
...
const createView = (isDark, expanded) => {
    if (view)
        view.destroy();
...
const contentHeight = (element) => {
    const childElement = element === null || element === void 0 ? void 0 : element.querySelector('.cm-content');
    if (childElement) {
        return (childElement === null || childElement === void 0 ? void 0 : childElement.offsetHeight) || 0;
    }
    return 0;
};
</script>
